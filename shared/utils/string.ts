...
export const unslugify = (url: string) => {
  return url
    .replace(/-+/g, " ") //replace spaces with delimeter
    .replace(/\s[a-z]/g, upperCase) //convert first char of each word to UPPERCASE
    .replace(/^[a-z]/g, upperCase); //convert first char to UPPERCASE
}
 ...
 */
export function contains(str: string, substring: string, fromIndex: number) {
 ...
 */
export function interpolate(template: string, replacements: string[], syntax: string) {
  var stache = /\{\{(\w+)\}\}/g; //mustache-like

  var replaceFn = function(match: any, prop: any) {
 ...
 */
export function truncate(str: string, maxChars: number, append: string, onlyFullWords: boolean) {
  append = append || "...";
...
export function address(str: string) {
  return str.substring(0, 5) + "..." + str.substring(37, 42);
}
